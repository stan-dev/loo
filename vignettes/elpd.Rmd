---
title: "Holdout and k-fold cross-validation of Stan programs with the loo package" 
...
%\VignetteIndexEntry{Holdout and k-fold cross-validation of Stan programs with the loo package}
-->
...
This vignette demonstrates how to do holdout and k-fold cross-validation with __loo__ for a Stan program.
...
See the demo for a description of the problem and data. 


## Coding the Stan model

Here is the Stan code for fitting the Poisson regression model:
...
For this approach, the model is first fit to the "train" data and then is evaluated on the "test" data.
...
## Fitting the model to the with RStan
...
```
And we draw samples from the generated quantities block based on the "train" data:

```{r gen-test}
gen_test <- gqs(stanmodel, draws = as.matrix(fit), data= data_test)
log_lik_test <- extract_log_lik(gen_test)
```
...
```{r elpd-holdout}
(elpd_holdout <- elpd(log_lik_test))
```
...
## Fitting and extracting the likelihood of each fold

We now loop over the 10 folds: Each time, we fit the model to all the observations except the ones belonging to the fold, but we extract the log-likelihood from the fold. The output of this loop is a list of log-likelihood matrices.

```{r}
log_lik_list <- lapply(1:10, function(k){
...
```{r elpd-kfold}
# First we need to convert the list of matrices into a long matrix:
log_lik_kfold <- do.call(cbind, log_lik_list) 
(elpd_kfold <- elpd(log_lik_kfold))
```

If one wants to compare several models (with `loo_compare`), one should use the same folds for all the different models. (Notice that now the likelihood values in `log_lik_kfold` are not in the same order as the observations of `roaches`.) 
