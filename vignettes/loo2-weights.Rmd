...
following four models (here we use here the default __rstanarm__ priors that are
weakly informative, while flat priors were used in _Statistical Rethinking_).
...
instead. As with the `waic` methods provided by the __loo__ package, the package
provides `loo` methods for log-likelihood arrays, matrices, and functions, but
since we fit our model with __rstanarm__ we can just pass the fitted model
objects directly and __rstanarm__ will extract the needed values to pass to the
__loo__ package.

```{r}
loo1 <- loo(fit1)  # accepts a 'cores' argument which is useful for bigger datasets
loo2 <- loo(fit2)
loo3 <- loo(fit3)
loo4 <- loo(fit4)
lpd_point <- cbind(
...
and 4 all $k<0.7$ and the Monte Carlo SE of `elpd_loo` is 0.1, and we can expect
...
optimistic. We recommend trusting the PSIS-LOO results instead.

To assess whether the contact rate and interaction term are useful, we can make
a comparison to models without these covariates.
...
lpd_point <- cbind(
  loo10p$pointwise[, "elpd_loo"], 
  loo11p$pointwise[, "elpd_loo"], 
  loo12p$pointwise[, "elpd_loo"]
)
```
...
Although asymptotically WAIC weights and Pseudo-BMA weights (without Bayesian
bootstrap) are equal, WAIC weights are likely to be more extreme in the finite
case, which is what we observe here. 
...
loo_list <- list(loo10p, loo11p, loo12p)
