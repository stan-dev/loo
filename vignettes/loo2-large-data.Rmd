...
LOO with subsampling and LOO using approximatations to posterior distributions. 
...
Gelman, and Gabry (2017)). In this case, the message tells us that all of the
...
We can now compare the models and estimate the difference based on the the same
subsampled observations.

```{r, eval=FALSE}
comp <- loo_compare(loo_ss_1, loo_ss_2_m)
print(comp) 
```
...
```{r, eval=FALSE}
loo_ss_2_full <- loo(x = llfun_logistic, draws = parameter_draws_2, data = stan_df_2)
comp <- loo_compare(loo_ss_1, loo_ss_2_full)
print(comp) 
```
