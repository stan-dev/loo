...
`model_compare()` function just as we would if using `loo()` instead of
...
We can now compare the models on LOO using the `model_compare` function:

```{r, eval=FALSE}
# Compare
comp <- model_compare(loo_ss_1, loo_ss_2)
print(comp)
```
...
model_compare(loo_ss_1, loo_full_2)
```
