...
$p(y_{i<M} \,|\, y_{<i})$ can be computed with the help of the posterior
distribution $p(\theta \,|\, y_{<i})$ of the parameters $\theta$ conditional on
...
$$
due to the assumption of conditional independence between $y_{i<M}$ and
$y_{<i}$ given $\theta_{<i}$.
...
$p(y_{i<M} \,|\, y_{<i})$ using Pareto smoothed importance sampling (PSIS,
...
than in LOO. For the present case study, we choose a threshold of $0.6$, but
more experiments are needed to give general recommendations about the threshold
in LFO.
...
ahead. In this case, $y_{i<M}$ simplifies to $y_{i}$ and the LFO-CV algorithm
...
left-out observations, which we set to $B = 10$ for the present case study.
