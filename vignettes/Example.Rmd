...
cross-validation and WAIC for evaluating fitted Bayesian models.* ([preprint here](http://arxiv.org/abs/1507.04544))
...
x <- scale(model.matrix(~ 0 + dist100 + arsenic, wells))
...
loo1 <- loo(log_lik1) # or waic(log_lik) to compute WAIC
```
