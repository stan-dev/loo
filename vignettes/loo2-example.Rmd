...
start demonstrating how it is useful also in model checking.
...
leave-one-out posteriors are so much different from the full posterior
...
times higher than the total number of of parameters in the model
...
there seems to be a block of data which is easier to predict (indices
...
about 7 which is a bit higher than the total number of of parameters
...
A reasonable guess is that that a hurdle or zero-inflated model is
needed.
