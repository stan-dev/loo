...
- Update documentation for
  [`E_loo()`](https://mc-stan.org/loo/reference/E_loo.md) function by
  [@avehtari](https://github.com/avehtari) in
...
- Fix bug in [`E_loo()`](https://mc-stan.org/loo/reference/E_loo.md)
  when type is variance by [@jgabry](https://github.com/jgabry) in
  [\#226](https://github.com/stan-dev/loo/issues/226)
- [`E_loo()`](https://mc-stan.org/loo/reference/E_loo.md) now allows
...
- improved [`E_loo()`](https://mc-stan.org/loo/reference/E_loo.md)
...
- New functions [`crps()`](https://mc-stan.org/loo/reference/crps.md),
  [`scrps()`](https://mc-stan.org/loo/reference/crps.md),
  [`loo_crps()`](https://mc-stan.org/loo/reference/crps.md), and
  [`loo_scrps()`](https://mc-stan.org/loo/reference/crps.md) for
...
- Fixed a bug in
  [`relative_eff.function()`](https://mc-stan.org/loo/reference/relative_eff.md)
...
- New generic function
  [`elpd()`](https://mc-stan.org/loo/reference/elpd.md) (and methods for
  matrices and arrays) for computing expected log predictive density of
  new data or log predictive density of observed data. A new vignette
...
- The log weights provided by the importance sampling functions
  ...
  [`tis()`](https://mc-stan.org/loo/reference/tis.md), and
  [`sis()`](https://mc-stan.org/loo/reference/sis.md) no longer have the
  largest log ratio subtracted from them when returned to the user. This
  should be less confusing for anyone using the
...
- Added support for standard importance sampling and truncated
  ...
  [`sis()`](https://mc-stan.org/loo/reference/sis.md) and
  [`tis()`](https://mc-stan.org/loo/reference/tis.md)).
  ([\#125](https://github.com/stan-dev/loo/issues/125))

- [`compare()`](https://mc-stan.org/loo/reference/compare.md) now throws
  a deprecation warning suggesting
  [`loo_compare()`](https://mc-stan.org/loo/reference/loo_compare.md).
...
- For WAIC, warnings are only thrown when running
  [`waic()`](https://mc-stan.org/loo/reference/waic.md) and not when
...
- New function
  ...
  [`compare()`](https://mc-stan.org/loo/reference/compare.md) function.
  ([\#93](https://github.com/stan-dev/loo/issues/93))
...
- New glossary page (use
  [`help("loo-glossary")`](https://mc-stan.org/loo/reference/loo-glossary.md))
...
- Improved stability of
  [`psis()`](https://mc-stan.org/loo/reference/psis.md) when
...
- New function [`psis()`](https://mc-stan.org/loo/reference/psis.md)
  replaces [`psislw()`](https://mc-stan.org/loo/reference/psislw.md)
...
- Updated the interface for the
  [`loo()`](https://mc-stan.org/loo/reference/loo.md) methods to
  ...
  [`help("loo")`](https://mc-stan.org/loo/reference/loo.md) has been
  updated to describe the new behavior.

- The structure of the objects returned by the
  [`loo()`](https://mc-stan.org/loo/reference/loo.md) function has also
  changed slightly, as described in the **Value** section at
  [`help("loo", package = "loo")`](https://mc-stan.org/loo/reference/loo.md).
...
- When comparing more than two models with
  [`compare()`](https://mc-stan.org/loo/reference/compare.md), the first
  column of the output is now the `elpd` difference from the model in
  the first row.
...
- Introduce the [`E_loo()`](https://mc-stan.org/loo/reference/E_loo.md)
  function for computing weighted expectations (means, variances,
  quantiles).
...
- warning messages are now issued by
  [`psislw()`](https://mc-stan.org/loo/reference/psislw.md) instead of
  `print.loo`
- [`print.loo()`](https://mc-stan.org/loo/reference/print.loo.md) shows
  a table of pareto k estimates (if any k \> 0.7)
- Add argument to
  [`compare()`](https://mc-stan.org/loo/reference/compare.md) to allow
  loo objects to be provided in a list rather than in `'...'`
...
- Remove model weights from
  [`compare()`](https://mc-stan.org/loo/reference/compare.md). In
  previous versions of **loo** model weights were also reported by
  [`compare()`](https://mc-stan.org/loo/reference/compare.md). We have
  removed the weights because they were based only on the point estimate
  of the elpd values ignoring the uncertainty. We are currently working
  on something similar to these weights that also accounts for
...
- S3 generics and `matrix` and `function` methods for both
  ...
  [`waic()`](https://mc-stan.org/loo/reference/waic.md). The matrix
