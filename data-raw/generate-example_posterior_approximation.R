...
generate_lr_data <- function(
  ...
  x <- mvtnorm::rmvnorm(
    n = N,
    mean = rep(0, D),
    sigma = solve(diag(D) + off_diag_cov - diag(D) * off_diag_cov) / N
  )
  if (scaled) {
    x <- scale(x)
  }
...
lm_indep_post <- generate_lr_data(
  D = 10,
  N = 1000,
  scaled = TRUE,
  sigma_noise = 1,
...
# Laplace test data
opt_independent <- rstan::optimizing(
  stm,
  data = lm_indep_post,
  draws = 5000,
  constrained = FALSE,
...
opt_correlated <- rstan::optimizing(
  stm,
  data = lm_indep_post,
  draws = 5000,
  constrained = FALSE,
...
b_independent <- as.matrix(opt_independent$theta_tilde[,
  1:(npar_independent - 1)
])
...
ll_independent <- log_lik_blinreg(
  b = b_independent,
  x = lm_indep_post$x,
  y = lm_indep_post$y,
  sigma = sigma_independent,
  par_unconstrained = TRUE
)
ll_correlated <- log_lik_blinreg(
  b = b_correlated,
  x = lm_cor_post$x,
  y = lm_cor_post$y,
  sigma = sigma_correlated,
  par_unconstrained = TRUE
)
...
advi_fullrank_correlated <- read.table(
  "data-raw/raw_data/output_lr_loo_fullrank_grad100_lr.cor.data1k.R.csv",
  header = TRUE,
  sep = ",",
  fill = TRUE
)
advi_fullrank_independent <- read.table(
  "data-raw/raw_data/output_lr_loo_fullrank_grad100_lr.nocor.data1k.R.csv",
  header = TRUE,
  sep = ",",
  fill = TRUE
)
advi_fullrank_normal <- read.table(
  "data-raw/raw_data/output_normal_model_fullrank_grad100_normal_data.R.csv",
  header = TRUE,
  sep = ",",
  fill = TRUE
)
advi_meanfield_correlated <- read.table(
  "data-raw/raw_data/output_lr_loo_meanfield_grad100_lr.cor.data1k.R.csv",
  header = TRUE,
  sep = ",",
  fill = TRUE
)
advi_meanfield_independent <- read.table(
  "data-raw/raw_data/output_lr_loo_meanfield_grad100_lr.nocor.data1k.R.csv",
  header = TRUE,
  sep = ",",
  fill = TRUE
)
advi_meanfield_normal <- read.table(
  "data-raw/raw_data/output_normal_model_fullrank_grad100_normal_data.R.csv",
  header = TRUE,
  sep = ",",
  fill = TRUE
)
log_lik_idx <- which(grepl(
  colnames(advi_meanfield_independent),
  pattern = "log_lik"
))
log_lik_idx_normal <- which(grepl(
  colnames(advi_meanfield_normal),
  pattern = "log_lik"
))
...
opt_normal <- rstan::optimizing(
  stnm,
  data = stan_normal_data,
  draws = 1000,
  constrained = FALSE,
  hessian = TRUE,
  seed = 4712
...
for (i in 1:ncol(log_lik)) {
  log_lik[, i] <-
    dnorm(
      x = stan_normal_data$y[i],
      mean = opt_normal$theta_tilde[, "mu"],
      sd = opt_normal$theta_tilde[, "sigma"],
      log = TRUE
    )
}
...
save(
  test_data_psis_approximate_posterior,
  file = "tests/testthat/data-for-tests/test_data_psis_approximate_posterior.rda"
)
