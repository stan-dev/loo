...
test_that("loo returns object with correct structure", {
  ...
  expect_equal(colnames(loo1$pointwise), c("elpd_loo", "mcse_elpd_loo", "p_loo", "looic", "leverage_pareto_k"))
  expect_equal(dim(loo1), dim(LLmat))
})

test_that("two pareto k values are equal", {
  expect_identical(loo1$pointwise[,"leverage_pareto_k"], loo1$diagnostics$pareto_k)
})
