...
#' }
#'
Combine <-function(log_lik_list, method, BB="Ture",BB_n=1000)
{
  ...
  if (method =="stacking"){
    ...
    if (method =="loo"){
      ...
      if(BB==T){
        w_loo2  <- bb_weight(lpd_point, BB_n)   #3) loo withs using BB sample
...
Select <-function(log_lik_list, BB="Ture",BB_n=1000, visualise="Ture")
{
  if (!is.logical(BB))
    stop("BB must be logical.")
  K<-length(log_lik_list)                #number of models
  ...
  if(BB==F){
    ...
    BB_weighting <- dirichlet_rng(BB_n, rep(1,N))
    best_count<-rep(0,K)
    for( bb in 1:BB_n){
...
#' @return A vector of best weights that minimize the levae-one-out log score.
...
#' @details \code{lpd_point} is a matrix of pointwise leave-one-out likelihood, which can be calculeted from  \code{\link{loo}}. It should be a \eqn{N} by \eqn{K}  matrix when Sample size is \eqn{N} and model number is \eqn{K}.   Bayesian bootstrap  takes into account the uncertainty of finite data points and regularize the weights making them go further away from 0 and 1.
#'
#' @seealso
#' \code{\link{loo}} for details on leave-one-out elpd estimation.
...
#' }
#'
bb_weight <- function(lpd_point, BB_n=1000) {
  K<-ncol(lpd_point)                #number of models
  if (K==1)
    stop("Only one model is fould.")
  temp<-matrix(NA,BB_n, K)
