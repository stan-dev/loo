...
#' computational steps. This package implements the fast and stable computations
#' for LOO and WAIC laid out in Vehtari, Gelman, and Gabry (2015). From existing
#' posterior simulation draws, we compute LOO using very good importance
...
#'
#' @section Details:
#' After fitting a Bayesian model we often want to measure its predictive
#' accuracy, for its own sake or for purposes of model comparison, selection, or
#' averaging. Cross-validation and information criteria are two approaches for
#' estimating out-of-sample predictive accuracy using within-sample fits.
#'
...
#' BUGS (Spiegelhalter, Best, et al., 2002; Spiegelhalter, Thomas, et al., 1994,
#' 2003), but it is known to have some problems, arising in part from it not
#' being fully Bayesian in that it is based on a point estimate (van der Linde,
#' 2005, Plummer, 2008). For example, DIC can produce negative estimates of the
...
#' package we present fast and stable computations for LOO and WAIC that can be
#' performed directly on posterior simulations, thus allowing these newer tools
#' to enter routine statistical practice. We compute LOO using very good
...
#' @section VGIS-LOO:
#'
#' We can improve the LOO estimate using very good importance sampling (VGIS;
#' Vehtari and Gelman, 2015), which applies a smoothing procedure to the
#' importance weights. We briefly review the motivation and steps of VGIS here.
#'
...
#' central limit theorem for stable distributions. Thus, even if the variance of
