...
loo_compare.psis_loo_ss_list <- function(x, ...) {
  ...
  for (i in seq_along(x)) {
    if (!inherits(x[[i]], "psis_loo_ss")) {
      x[[i]] <- as.psis_loo_ss.psis_loo(x[[i]])
    }
  }
  ...
  for (i in 2:length(ord)) {
    elpd_diff_mat[i, ] <- loo_compare_ss(
      ref_loo = x[ord[1]],
      compare_loo = x[ord[i]]
    )
  }
...
loo_compare_ss <- function(ref_loo, compare_loo) {
  ...
  # Using HH estimation
  if (
    ref_loo[[1]]$loo_subsampling$estimator == "hh_pps" |
      compare_loo[[1]]$loo_subsampling$estimator == "hh_pps"
  ) {
    warning(
      "Hansen-Hurwitz estimator used. Naive diff SE is used.",
      call. = FALSE
    )
  ...
  if (compare_subset_of_ref | ref_subset_of_compare) {
    if (compare_subset_of_ref) {
      ref_loo[[1]] <- update(
        object = ref_loo[[1]],
        observations = compare_loo[[1]]
      )
    }
    if (ref_subset_of_compare) {
      compare_loo[[1]] <- update(compare_loo[[1]], observations = ref_loo[[1]])
    }
    message(
...
loo_compare_ss_naive <- function(ref_loo, compare_loo) {
  ...
  elpd_loo_diff <- ref_loo[[1]]$estimates["elpd_loo", "Estimate"] -
    compare_loo[[1]]$estimates["elpd_loo", "Estimate"]
...
loo_compare_ss_diff <- function(ref_loo, compare_loo) {
  ...
  checkmate::assert_true(identical(
    obs_idx(ref_loo[[1]]),
    obs_idx(compare_loo[[1]])
  ))

  # Assert not none as loo approximation
  checkmate::assert_true(
    ref_loo[[1]]$loo_subsampling$loo_approximation != "none"
  )
  checkmate::assert_true(
    compare_loo[[1]]$loo_subsampling$loo_approximation != "none"
  )

  diff_approx <- ref_loo[[1]]$loo_subsampling$elpd_loo_approx -
    compare_loo[[1]]$loo_subsampling$elpd_loo_approx
  diff_sample <- ref_loo[[1]]$pointwise[, "elpd_loo"] -
    compare_loo[[1]]$pointwise[, "elpd_loo"]
  est <- srs_diff_est(
    diff_approx,
    y = diff_sample,
    y_idx = ref_loo[[1]]$pointwise[, "idx"]
  )
