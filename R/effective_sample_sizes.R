...
#'   \code{x} if \code{x} is a function) should be likelihood values (i.e.,
#'   \code{exp(log_lik)}). For generic \code{use} with \code{\link{psis}}, the
#'   values in \code{x} should be the reciprocal of the importance ratios (i.e.,
#'   \code{exp(-log_ratios)}).
...
#'
relative_eff.function <- function(x, chain_id, ..., data, draws) {
  ...
  vapply(
    ...
    FUN = function(i) {
      ...
      lik_i <- exp(as.vector(ll_i))
      relative_eff.default(lik_i, chain_id = chain_id)
    },
    FUN.VALUE = numeric(1)
  )
}
