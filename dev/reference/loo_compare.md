...
[`se_diff`](https://mc-stan.org/loo/dev/reference/loo-glossary.md)
columns of the returned matrix are computed by making pairwise
comparisons between each model and the model with the largest ELPD (the
model in the first row). For this reason the `elpd_diff` column will
always have the value `0` in the first row (i.e., the difference between
the preferred model and itself) and negative values in subsequent rows
