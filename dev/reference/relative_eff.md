...
- x:
  ...
  - For use with the
    ...
    the values in `x` (or generated by `x`, if a function) should be
    **likelihood** values (i.e., `exp(log_lik)`), not on the log scale.

  - For generic use with
    [`psis()`](https://mc-stan.org/loo/dev/reference/psis.md), the
    values in `x` should be the reciprocal of the importance ratios
    (i.e., `exp(-log_ratios)`).
