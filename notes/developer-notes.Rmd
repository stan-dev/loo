...
- **Options:** `lower_is_better`, `orientation = "utility" / "loss"`,
  `higher_is_better` (per-measure orientation control on `measure_*()` functions)
- **Decision:** *pending*
...
|------------|--------------|-------|
| `crps(x, x2, y)` | `measure_rps(y, ypred = x, higher_is_better = FALSE)` | Loss-scale unscaled score |
...
1. **Sign convention (unscaled only).** `crps()` returns `0.5·EXX − EXy` (utility:
   ...
   `higher_is_better = FALSE` to match `crps()`. Scaled scores (`scrps` / `measure_srps`)
...
new_rps <- measure_rps(y, x1, higher_is_better = FALSE)
...
`measure_rps(higher_is_better = FALSE)`; right — `scrps()` vs `measure_srps()`.*
...
for (i in seq_len(n_repl)) {
  ...
  new <- measure_rps(y, x1, higher_is_better = FALSE)
...
.outcome_panel(
  crps_old, rps_new,
  xlab = "crps() estimate",
  ylab = "measure_rps(higher_is_better = FALSE)",
